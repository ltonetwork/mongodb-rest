...
MongoDbRest.prototype.returnDocuments = function (request, response, id, docs) {
  ...
  if (docs!="") {
    ...
    self.emit("log", request, response, 200);
  } else {
    if (id) {
      response.writeHead(404, {});
      response.body = "";
      self.emit("log", request, response, 404);
    } else {
      docs = [];
      response.body = JSON.stringify(docs);
      response.writeHead(200, { 
...
MongoDbRest.prototype.doQuery = function (request, response, parameters) {
  ...
  if (parameters['id']) {
    query = {'_id': new mongo.BSONPure.ObjectID(parameters['id'])};
  }
  ...
  db.open(function(err,db) {
    db.collection(parameters['collection'], function(err, collection) {
      collection.insert(parameters['document'], function(err, docs) {
        ...
        response.body = MongoDbRest.MONGO_YES;
        response.write(response.body);
        self.emit("log", request, response, 201);
        response.end();
        db.close();
      });
    });
...
MongoDbRest.prototype.updateDocument = function (request, response, parameters) {
  var self = this;
  var spec = {'_id': new mongo.BSONPure.ObjectID(parameters['id'])};
...
MongoDbRest.prototype.doCommand = function (request, response, parameters) {
  ...
  switch (parameters['command']) {
    ...
    default:
      ...
      response.body = MongoDbRest.MONGO_NO;
      response.write(response.body);
      self.emit("log", request, response, 501);
      response.end();
      break;
  }
};
...
MongoDbRest.prototype.handleRequest = function (request, response) {
  ...
  switch (request.method) {
    case "GET":
      if (parameters['db'] && parameters['db'] != 'favicon.ico') {
...
MongoDbRest.prototype.start = function() {
  ...
  self.emit("listening", self.config['server']['address'], self.config['server']['port']);
};
