...
/**
 * Update
 * :query is used with :id=0
 */
app.put('/:db/:collection/:id/:query', function(req, res) {
  var spec,
      multi = false;
  ...
  if(req.params.id != "0") {
    ...
    spec = JSON.parse(req.params.query);
    multi = true;
  }
  ...
  db.open(function(err, db) {
    db.authenticate(config.db.username, config.db.password, function () {
      db.collection(req.params.collection, function(err, collection) {
        collection.update(spec, req.body, {upsert : true, multi : multi}, function(err, docs) {
          res.header('Content-Type', 'application/json');
          res.send('{"ok":1}');
          db.close();
        });
