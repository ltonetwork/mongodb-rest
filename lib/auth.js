...
module.exports = function(app, config) {
    ...
    app.use( function(req, res, next) {
        ...
        MongoClient.connect(config.auth.tokenDBConnection, function (err, tokensDb) {
            ...
            tokensDb.collection('tokens', function (err, tokensCollection) {
                if (err) {
                    tokensDb.close();
                    res.status(500).json({ error: err });
                    return;
    ...
    app.use( '/login', function(req, res) {
        ...
        if (!email) {
            res.status(400).json({ message: '"email" not specified in request body.'});
            return;
        }
        ...
        MongoClient.connect(config.auth.usersDBConnection, function (err, usersDb) {
            ...
            usersDb.collection('users', function (err, usersCollection) {
                ...
                usersCollection.findOne({ email: email }, function (err, user) {
                    ...
                    if (err) {
                        ...
                        res.status(401).json({ message: 'Incorrect password for user with email ' + email });
                        return;
                    }
