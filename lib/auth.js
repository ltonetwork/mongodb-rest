...
module.exports = function(app, config) {
    ...
    config.tokenExpirationTimeHours == config.tokenExpirationTimeHours || DEFAULT_TOKEN_EXPIRATION_TIME_HOURS;

    var usersDBConnection = mongoose.createConnection( config.usersDbMongoUri ),
        tokenDBConnection = mongoose.createConnection( config.tokenDbMongoUri );
    ...
    app.use( function(req, res, next) {
        ...
        if( req.query.token == undefined ) {
            ...
            if( config.universalAuthToken !== undefined && req.query.token == config.universalAuthToken ) {
                return next();
            }            
            ...
            Token.findOne({token: req.query.token}, function(err, token) {
                if( token ) {
                    ...
                    createdDateCopy.setHours( createdDateCopy.getHours() + config.tokenExpirationTimeHours );
